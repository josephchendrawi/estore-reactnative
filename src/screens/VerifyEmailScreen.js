...
class VerifyEmailScreen extends Component {
  ...
  doSendingOTP = () => {
    ...
    API.GetSendOtpEmailRequest(this.props.email)
    .then((response) => {
  ...
  doVerification = () => {
    if(this.state.otp == null){
      ...
      API.GetCustomerVerifyEmailRequest(this.props.email, this.state.otp)
      .then((response) => {
        if(response.data.sts == 0){
          if(this.props.generalInfo.needPhoneVerify == true){
            Actions.verifyphone();
          }
          else{
    ...
    return (
      <SafeAreaView style={[globalStyle.container, { justifyContent: 'center', alignItems: 'center' }]}>
        <View style={{alignItems: 'center'}}>
          <Text style={{color: colorPalette.defaultText, fontSize: 20}}>Please verify your email.</Text>
        </View>
